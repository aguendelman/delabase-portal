...
interface YearComparison {
  year1: { year: number; period?: string; totalSales: number; totalValue: number };
  year2: { year: number; period?: string; totalSales: number; totalValue: number };
...
// Opciones de período incluyendo trimestres
const periodOptions = [
  ...
  { value: 'Q1', label: 'Q1 (EFM)' },
  { value: 'Q2', label: 'Q2 (AMJ)' },
  { value: 'Q3', label: 'Q3 (JAS)' },
  { value: 'Q4', label: 'Q4 (OND)' },
];
...
export default function ReportesPage() {
  ...
  useEffect(() => {
    loadReport();
  }, [selectedReport, selectedYear, monthsThreshold, compYear1, compPeriod1, compYear2, compPeriod2]);
  ...
  const loadReport = async () => {
    ...
    try {
      switch (selectedReport) {
        ...
        case 'comparison':
          const period1 = compPeriod1 || 'full';
          const period2 = compPeriod2 || 'full';
  ...
  const renderComparisonReport = () => {
    ...
    // Función para formatear la etiqueta del período
    const formatPeriodLabel = (year: number, period?: string) => {
      if (!period) {
        return `${year}`;
      }
      // Si es un trimestre
      if (period.startsWith('Q')) {
        const quarterLabels: Record<string, string> = { 'Q1': 'EFM', 'Q2': 'AMJ', 'Q3': 'JAS', 'Q4': 'OND' };
        return `${quarterLabels[period] || period} ${year}`;
      }
      // Si es un mes
